...
/**
 * Wrap elvis and sectra specific booking classes in order to generalize their return.
 * This would enable the BookingFacade to represent both of them and to in turn be utilized by the presentation layer.
 * @author carstm
 *
 */


//adapt these var names as needed to fit existing xhtml code and vars there in, example: name changed from name to displayName
...
//		this.pnr			= bl.getPatientId();
	...
	public BookingWrapper getBookingInfo(State state) {
		if(state.getService().equals("MAMMO_SU")){
			...
			SectraBookingFacadeImpl facade = new SectraBookingFacadeImpl();
			BookingWrapper wrapper = new BookingWrapper();
			System.out.println("### BookingWrapper.getBookingInfo(state). State pnr & passw: " + state.getPnr() + ", " + state.getPasswd());
			wrapper = facade.getBookingInfo(state);
			System.out.println("### returning wrapper after getBookingInfo call");
			return wrapper;
		}
